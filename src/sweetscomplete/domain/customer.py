...
class CustomerService(Base) :
    ...
    """
    Generates and returns new password for this customer
    Updates database with new password
    @param string custKey : customer key
    @param string newPass : new password
    """
    def newPassword(self, custKey, newPass) :
        import bcrypt
        # hash using bcrypt
        hashPass = bcrypt.hashpw(newPass.encode('utf-8'),bcrypt.gensalt())
        # append to CSV file
        data  = {"password" : str(hashPass)}
        return self.editOneByKey(custKey, data)
